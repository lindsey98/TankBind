...
if __name__ == '__main__':
    ...
    info = pd.read_csv(f"{base_pre}/result_info.csv")
    chosen = info.loc[info.groupby(['protein_name', 'smiles'], sort=False)['affinity'].agg('idxmax')].reset_index()
    affinity_values = chosen['affinity'].tolist()

    # Create the histogram
    plt.hist(affinity_values, bins=30, edgecolor='black')  # You can adjust the number of bins
    ...
    data_with_batch_info = next(iter(DataLoader(dataset[idx:idx+1], batch_size=1,
                                    ...
                                    shuffle=False,
                                    num_workers=1)))
    data_with_batch_info = data_with_batch_info.to(device)
